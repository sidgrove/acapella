...
public sealed class ContinuationRuleTests
{
    ...
    public void A_new_sentence_after_a_dropped_stop_gets_the_stop_back() =>
        ContinuationRule.Prefix(Previous("we just need to be prudent"), "And have good understanding", "A/1", 5, Then.AddSeconds(9)).ShouldBe(". ");

    [Fact]
    public void A_lower_case_continuation_gets_a_space() =>
        ContinuationRule.Prefix(Previous("the bottom line"), "by financing literally", "A/1", 5, Then.AddSeconds(9)).ShouldBe(" ");

    [Fact]
    public void A_question_kept_its_mark_so_only_a_space_is_needed() =>
        ContinuationRule.Prefix(Previous("how's that going?", stopDropped: false), "The Claude reset", "A/1", 5, Then.AddSeconds(9)).ShouldBe(" ");
    ...
    public void Nothing_is_typed_when_the_field_or_the_moment_has_moved_on(string target, long presses, int seconds, bool enter) =>
        ContinuationRule.Prefix(Previous("done", enter: enter), "Next", target, presses, Then.AddSeconds(seconds)).ShouldBe(string.Empty);

    [Fact]
    public void The_first_dictation_of_a_session_has_nothing_to_join() =>
        ContinuationRule.Prefix(null, "Hello", "A/1", 0, Then).ShouldBe(string.Empty);

    [Fact]
    public void An_unknown_target_is_never_joined() =>
        ContinuationRule.Prefix(Previous("done", target: null), "Next", null, 5, Then.AddSeconds(1)).ShouldBe(string.Empty);
    ...
    public void Whitespace_and_punctuation_at_the_join_are_left_to_the_user(string previous, string next) =>
        ContinuationRule.Prefix(Previous(previous), next, "A/1", 5, Then.AddSeconds(1)).ShouldBe(string.Empty);
}
