...
public sealed class SettingsView : UserControl
{
    ...
    private StackPanel BuildAiSection()
    {
        ...
        return Panels.Column(Tokens.Space.Base,
            ...
            Panels.SwitchRow("Let the clean-up see where you're typing",
                "Sends the app's name, its window title and up to 1,000 characters before the cursor along with the dictation, so a name already in the email or chat you're answering is spelt the same way. Only used for spelling; nothing else changes.",
            ...
            Panels.SwitchRow("Match the writing to the app",
                "Lays the text out for where it's going: an email gets short paragraphs and your greeting and sign-off on their own lines, a Slack or Teams message stays one message, a prompt to Claude or ChatGPT keeps file names and code as written. Your words stay yours. Apps it doesn't know are judged by Jev while you talk, if it has a key.",
            ...
            Text.Muted("TypeSafe's Jev answers small yes/no questions in about a tenth of a second: was that a send command, does this sentence carry on, is “period” the noun. It decides whether a fix you make was a mishearing before it's learnt, and what kind of writing an app it doesn't know is for; its other answers only go to the log. Nothing you're waiting on waits for it. Leave the key blank to switch it off."),
    ...
    private StackPanel BuildBehaviourSection()
    {
        var column = Panels.Column(Tokens.Space.Roomy,
            ...
            Panels.SwitchRow("Keep recordings of what I correct",
                "Keeps the audio of a dictation only when you change its words afterwards, so the speech model can one day be tuned to your voice on the mistakes that matter. Everything else is deleted once the edit check is done. Needs “Learn from my edits”. Nothing is uploaded.",
                _settings.Data.KeepRecordings, v => Save(_settings.Data with { KeepRecordings = v })),
            Panels.SwitchRow("Learn from my edits",
                "For two minutes after typing, or until you send it or move away, reads the box back to see which words you changed. The changes are kept in the history to measure accuracy, and a word you fix to something that sounds like it is learnt for the dictionary. The text stays on this PC; only the fixed words and a line either side go to Jev, if it has a key.",
            ...
            Panels.SwitchRow("Add fixes to the dictionary by themselves",
                "When Jev is sure a fix was a mishearing, or you make the same fix twice, it goes straight into the dictionary and shows under “Learnt from your edits” with an Undo. Off leaves every fix as a suggestion for you to add.",
